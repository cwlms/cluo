...
class MasterMatchStageV3_4(Stage):
    ...
    def __init__(
        ...
        record_array_hash_field: str = "array_hash",
        ...
        Args:
            ...
            record_array_hash_field (str, optional): The name of the field in the record data that contains the array hash. Defaults to `"array_hash"`.
        ...
        self.record_array_hash_field = record_array_hash_field
        ...
        return sql.SQL(
            dedent(
                ...
                with matches as materialized (
                        select {record_data_source_key} as data_source_key -- incoming record key
                        union
                        select distinct unnest(data_source_keys) as data_source_key -- get hard matches from master
                        from master.{master_table_name}
                        where array[{record_data_source_key}::text] && data_source_keys
                        union
                        select distinct unnest(mh.data_source_keys) as data_source_key -- get hash matches to master
                        from master.{master_table_name} mh
                        where mh.array_hash && {array_hash}::_text
                        and cardinality(array(select unnest({array_hash}::_text) intersect select unnest(mh.array_hash)))::decimal/cardinality({array_hash}::_text) >= {match_ratio})
                select json_agg(c.*)::text as matches -- return all clean records that matched to the incoming record
                from clean.{master_table_name} c
                where c.data_source_key in (select data_source_key from matches)
                    and (c.ts_status is null or c.ts_status != 'DELETED')
                ;
                """
            )
        ).format(
            ...
            array_hash=sql.Literal(record.data[self.record_array_hash_field]),
        )
